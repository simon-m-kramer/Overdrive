...
#include "Starter/TimeTrialPlayerController.h"
#include "Starter/TimeTrialUI.h"
#include "Engine/World.h"
#include "Starter/TimeTrialGameMode.h"
#include "Starter/TimeTrialTrackGate.h"
#include "EnhancedInputSubsystems.h"
#include "Engine/LocalPlayer.h"
#include "InputMappingContext.h"
#include "Starter/OverdriveUI.h"
	...
	if (IsLocalPlayerController())
	{
		if (ShouldUseTouchControls())
		{
			...
			if (MobileControlsWidget)
			{
				...
				UE_LOG(LogOverdrive, Error, TEXT("Could not spawn mobile controls widget."));
		...
		if (UIWidget)
		{
			...
			UE_LOG(LogOverdrive, Error, TEXT("Could not spawn Time Trial UI widget."));

		}
		

		// spawn the UI widget and add it to the viewport
		VehicleUI = CreateWidget<UOverdriveUI>(this, VehicleUIClass);
		...
		if (VehicleUI)
		{
			...
			UE_LOG(LogOverdrive, Error, TEXT("Could not spawn vehicle UI widget."));
...
void ATimeTrialPlayerController::OnPossess(APawn* InPawn)
{
	...
	VehiclePawn = CastChecked<AOverdrivePawn>(InPawn);
...
void ATimeTrialPlayerController::OnPawnDestroyed(AActor* DestroyedPawn)
{
	...
	if (ActorList.Num() > 0)
	{
		...
		if (AOverdrivePawn* RespawnedVehicle = GetWorld()->SpawnActor<AOverdrivePawn>(VehiclePawnClass, SpawnTransform))
		{
			// possess the vehicle
